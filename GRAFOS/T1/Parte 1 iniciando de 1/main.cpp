...
void exibirMatriz(const vector<vector<int>>& matrizAdj) {
    ...
    for (int i = 0; i < n; ++i) {
        cout << " " << i+1; // Cabeçalho das colunas
    }
    cout << endl;
    ...
    for (int i = 0; i < n; ++i) {
        cout << i+1 << " |"; // Número da linha e barra vertical para separar
        for (int j = 0; j < n; ++j) {
            cout << " " << matrizAdj[i][j];
        }
        cout << endl;
...
void bfs(const vector<vector<int>>& matrizAdj, int inicio, int n, int elemento = -1) {
    ...
    auto bfs_aux = [&](int v) {
        ...
        while (!fila.empty()) {
            ...
            if (vertice == elemento) { // Verifica se o vértice atual é o elemento procurado
                cout << "\nElemento " << elemento+1 << " encontrado!\n";
                return true;
            }
    ...
    if (elemento != -1 && find(sequenciaBusca.begin(), sequenciaBusca.end(), elemento) == sequenciaBusca.end()) {
        cout << "Elemento " << elemento+1 << " não encontrado.\n";
    }
}
...
int main() {
    ...
    while (true) {
        ...
        if (opcao == 1) { // Adicionar um novo vértice ao grafo
            ...
            cout << "Digite o vértice a ser removido (1 a " << n << "): ";
            int verticeRemover;
            cin >> verticeRemover;
            verticeRemover--;
