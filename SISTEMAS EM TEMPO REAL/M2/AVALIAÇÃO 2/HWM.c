...
// Estruturas para armazenar o HWM e skip
static uint64_t tempos_execucao_injecao[AMOSTRAS_TEMPO] = {0};
static int cont_ativacoes_injecao = 0;
static int cont_sucesso_injecao = 0;
...
// Função para verificar HWM e (m,k)-firme
void verificar_hw_firme(uint64_t exec_time, int *cont_ativacoes, int *cont_sucesso, uint64_t *hwm, uint64_t deadline) {
    // Atualiza o HWM se o tempo de execução for maior
    if (exec_time > *hwm) {
        *hwm = exec_time;
    }

    // Incrementa a contagem de ativações na janela
    (*cont_ativacoes)++;

    // Conta ativações que atendem ao deadline
    ...
    if (*cont_ativacoes == JANELA_ATIVACOES) {
        int skip_count = JANELA_ATIVACOES - *cont_sucesso;
        printf("HWM: %llu us, Atendidas: %d, Skip: %d/%d!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", *hwm, *cont_sucesso, skip_count, JANELA_ATIVACOES);
...
static void verificar_injecao_eletronica(void *pvParameter)
{
    ...
    while (1) {
        ...
        if (s_pad_activated[TOUCH_INJECAO] == true) {
            ...
            // Armazena a execução em tempos_execucao_injecao
            tempos_execucao_injecao[cont_ativacoes_injecao % AMOSTRAS_TEMPO] = tempo_execucao;
            ...
            // Verifica HWM e (m,k)-firme para essa tarefa
            verificar_hw_firme(tempo_execucao, &cont_ativacoes_injecao, &cont_sucesso_injecao, &WCET_injecao, DEADLINE_INJECAO);
...
void app_main(void)
{
    ...
    xTaskCreate(verificar_injecao_eletronica, "InjecaoEletronicaTask", 2048, NULL, 7, NULL);
    xTaskCreate(verificar_temperatura_motor, "TemperaturaMotorTask", 2048, NULL, 3, NULL);
    xTaskCreate(verificar_abs, "ABSTask", 2048, NULL, 12, NULL);
    xTaskCreate(verificar_airbag, "AirbagTask", 2048, NULL, 20, NULL);
